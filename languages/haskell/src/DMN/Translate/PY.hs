...
toPY :: JSOpts -> DecisionTable -> String
...
mkFunction tablename = [ "def", underscore tablename ]
...
fexp2js jsopts ch fexps = wrapParen " or " (feel2jsIn ( showVarname jsopts ch) <$> fexps)
...
feel2jsIn lhs (FInRange lower upper)   = wrapParen " and " [show lower ++ "<=" ++ lhs, lhs ++ "<=" ++ show upper]
...
showFeel (FInRange lower upper)   = "(x)=>" ++ show lower ++ "<= x and x <= " ++ show upper
